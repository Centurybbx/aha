...
from aha.bus import InboundMessage, MessageBus
...
class DiscordChannel(BaseChannel):
    ...
    async def start(self) -> None:
        try:
            ...
            raise RuntimeError("discord dependency not installed, run: pip install 'aha-agent[discord]'") from exc
        ...
        self._client_task = asyncio.create_task(client.start(self.token), name="aha.discord.client")
        for _ in range(100):
            if client.is_ready():
                break
